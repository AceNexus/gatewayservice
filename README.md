...
微服務生態系的統一入口，基於 Spring Cloud Gateway。透過 JWT 驗證保護所有 API，將請求路由至對應微服務，並透過 Spring Cloud
Bus + RabbitMQ 支援動態配置刷新。
...
1. 在專案開發機編譯 JAR：
   ...
   ```
2.
將 `build/libs/gatewayservice.jar`、`docs/docker/Dockerfile`、`docs/docker/.dockerignore`、`docs/docker/docker-compose.yml`、`docs/docker/.env.example`
複製到部署主機的同一目錄
...
[release] gradle bootJar
          ...
          → push ghcr.io/acenexus/gatewayservice:<sha> + :latest
          → 更新 deploy repo k8s/gatewayservice/deployment.yaml 的 image tag
